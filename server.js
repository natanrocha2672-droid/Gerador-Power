...
async function handlePexels(req,res){const u=new URL(req.url,`http://${req.headers.host}`);if(u.pathname==='/api/pexels/status'){const k=pexelsKey();return sendJson(res,200,{configured:!!k,length:k.length,prefix:k?k.slice(0,4):'',note:'O valor completo nunca é exposto.'})}return sendJson(res,200,await searchPexels(u.searchParams.get('query')||'embroidery',u.searchParams.get('per_page')||1),'public, max-age=86400')}
async function makeSpeechBuffer(input,voice,speed){const key=hash(JSON.stringify({input,voice,speed,model:process.env.OPENAI_TTS_MODEL||'gpt-4o-mini-tts'}));if(audioCache.has(key))return {buffer:audioCache.get(key),cached:true,key};const r=await fetch('https://api.openai.com/v1/audio/speech',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_TTS_MODEL||'gpt-4o-mini-tts',input,voice,response_format:'mp3',speed,instructions:'Português do Brasil natural e didático.'})});if(!r.ok)throw new Error('Erro TTS '+r.status);const buffer=Buffer.from(await r.arrayBuffer());audioCache.set(key,buffer);if(audioCache.size>60)audioCache.delete(audioCache.keys().next().value);return {buffer,cached:false,key}}
async function handleTTS(req,res){if(!process.env.OPENAI_API_KEY)return sendJson(res,500,{error:'OPENAI_API_KEY não configurada'});if(req.method==='GET'){const u=new URL(req.url,`http://${req.headers.host}`);if(u.pathname==='/api/tts/status')return sendJson(res,200,{cached:audioCache.size,mode:'memory-cache'});return sendJson(res,405,{error:'Use POST para gerar áudio'})}if(req.method!=='POST')return sendJson(res,405,{error:'Use POST'});let p;try{p=JSON.parse(await readBody(req))}catch{return sendJson(res,400,{error:'JSON inválido'})}const input=clean(p.text,4096),voice=p.voice||'coral',speed=Number(p.speed)||.95;try{const result=await makeSpeechBuffer(input,voice,speed);res.writeHead(200,{'Content-Type':'audio/mpeg','Cache-Control':'public, max-age=86400','X-Audio-Cache':result.cached?'HIT':'MISS','X-Audio-Key':result.key.slice(0,16)});res.end(result.buffer)}catch(e){sendJson(res,500,{error:e.message})}}
...
async function handleAtelier(req,res){if(req.method!=='POST')return sendJson(res,405,{error:'Use POST'});if(!process.env.OPENAI_API_KEY)return sendJson(res,500,{error:'OPENAI_API_KEY não configurada'});let p;try{p=JSON.parse(await readBody(req))}catch{return sendJson(res,400,{error:'JSON inválido'})}const m=p.module||{},ctx=`Módulo: ${clean(m.title,200)}. Resumo: ${clean(m.summary,1000)}. Conteúdo: ${clean(m.content,2500)}. Pergunta: ${clean(p.prompt,1200)}.`;try{const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-5.5',instructions:'Você é uma professora especialista em bordado. Responda em português do Brasil com passos práticos e use apenas o contexto do módulo quando possível.',input:ctx})});const d=await r.json();if(!r.ok)return sendJson(res,r.status,{error:d?.error?.message||'Erro OpenAI'});return sendJson(res,200,{text:extract(d)||'Não consegui responder agora.'})}catch(e){return sendJson(res,500,{error:e.message})}}
...
http.createServer((req,res)=>{if(req.url.startsWith('/api/course/'))return handleCourse(req,res);if(req.url.startsWith('/api/pexels'))return handlePexels(req,res);if(req.url.startsWith('/api/tts'))return handleTTS(req,res);if(req.url.startsWith('/api/atelier'))return handleAtelier(req,res);serve(req,res)}).listen(PORT,()=>console.log('Site rodando em http://localhost:'+PORT));
